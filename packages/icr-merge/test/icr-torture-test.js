// ICR TORTURE TEST â€” industry-standard property-based + adversarial testing of the
...
//                        auto result with itself returns it unchanged â€” absorption)
...
// deterministic PRNG (mulberry32) â€” reproducible: re-run with the printed seed
function rng(seed) { let a = seed >>> 0; return () => { a |= 0; a = (a + 0x6D2B79F5) | 0; let t = Math.imul(a ^ (a >>> 15), 1 | a); t = (t + Math.imul(t ^ (t >>> 7), 61 | t)) ^ t; return ((t ^ (t >>> 14)) >>> 0) / 4294967296 } }
const rand = rng(SEED)
const pick = (arr) => arr[Math.floor(rand() * arr.length)]
let SENT = 100000
const sentinel = () => ++SENT // unique integer per edit â€” greppable in output
...
for (const [label, gen] of Object.entries(GEN)) {
  ...
  for (let it = 0; it < ITERS; it++) {
    ...
    if (r1.status === 'auto') {
      autoN++
      // P1 parse guarantee â€” re-merge trivially to invoke provider parse; also absorption
...
function directed(name, file, base, a, b, check) {
  ...
  assert(`${name} â€” symmetric`, r.status === r2.status && (r.status !== 'auto' || r.text === r2.text))
  check(r)
...
directed('PY unicode-names', 'x.py',
  ...
  `def è®¡ç®—():\n    return 1\n\ndef Ð¿Ñ€Ð°Ð²Ð´Ð°():\n    return 2\n`,
  `def è®¡ç®—():\n    return 1\n\ndef Î´Î¿ÎºÎ¹Î¼Î®():\n    return 3\n`,
  (r) => { assert('PY unicode: never corrupts', r.status !== 'auto' || (r.text.includes('Ð¿Ñ€Ð°Ð²Ð´Ð°') && r.text.includes('Î´Î¿ÎºÎ¹Î¼Î®'))) })
